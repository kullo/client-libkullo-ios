// AUTOGENERATED FILE - DO NOT MODIFY!
// This file generated by Djinni from api.djinni

#import "KADrafts+Private.h"
#import "KADrafts.h"
#import "DJICppWrapperCache+Private.h"
#import "DJIError.h"
#import "DJIMarshal+Private.h"
#include <exception>
#include <utility>

static_assert(__has_feature(objc_arc), "Djinni requires ARC to be enabled for this file");

@interface KADrafts ()

- (id)initWithCpp:(const std::shared_ptr<::Kullo::Api::Drafts>&)cppRef;

@end

@implementation KADrafts {
    ::djinni::DbxCppWrapperCache<::Kullo::Api::Drafts>::Handle _cppRefHandle;
}

- (id)initWithCpp:(const std::shared_ptr<::Kullo::Api::Drafts>&)cppRef
{
    if (self = [super init]) {
        _cppRefHandle.assign(cppRef);
    }
    return self;
}

- (nonnull NSString *)text:(int64_t)convId {
    try {
        auto r = _cppRefHandle.get()->text(::djinni::I64::toCpp(convId));
        return ::djinni::String::fromCpp(r);
    } DJINNI_TRANSLATE_EXCEPTIONS()
}

- (void)setText:(int64_t)convId
           text:(nonnull NSString *)text {
    try {
        _cppRefHandle.get()->setText(::djinni::I64::toCpp(convId),
                                     ::djinni::String::toCpp(text));
    } DJINNI_TRANSLATE_EXCEPTIONS()
}

- (KADraftState)state:(int64_t)convId {
    try {
        auto r = _cppRefHandle.get()->state(::djinni::I64::toCpp(convId));
        return ::djinni::Enum<::Kullo::Api::DraftState, KADraftState>::fromCpp(r);
    } DJINNI_TRANSLATE_EXCEPTIONS()
}

- (void)prepareToSend:(int64_t)convId {
    try {
        _cppRefHandle.get()->prepareToSend(::djinni::I64::toCpp(convId));
    } DJINNI_TRANSLATE_EXCEPTIONS()
}

- (void)clear:(int64_t)convId {
    try {
        _cppRefHandle.get()->clear(::djinni::I64::toCpp(convId));
    } DJINNI_TRANSLATE_EXCEPTIONS()
}

namespace ObjCpp { namespace Kullo { namespace Api {

auto Drafts::toCpp(ObjcType objc) -> CppType
{
    if (!objc) {
        return nullptr;
    }
    return objc->_cppRefHandle.get();
}

auto Drafts::fromCpp(const CppType& cpp) -> ObjcType
{
    if (!cpp) {
        return nil;
    }
    return ::djinni::DbxCppWrapperCache<::Kullo::Api::Drafts>::getInstance()->get(cpp, [] (const CppType& p) {
        return [[KADrafts alloc] initWithCpp:p];
    });
}

} } }  // namespace ObjCpp::Kullo::Api

@end
