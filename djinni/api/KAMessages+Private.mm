...
@implementation KAMessages {
    ::djinni::DbxCppWrapperCache<::Kullo::Api::Messages>::Handle _cppRefHandle;
}
...
- (nonnull NSArray *)allForConversation:(int64_t)convId {
    try {
...
- (nonnull NSArray *)deliveryState:(int64_t)msgId {
    try {
...
auto Messages::fromCpp(const CppType& cpp) -> ObjcType
{
    if (!cpp) {
        return nil;
    }
    return ::djinni::DbxCppWrapperCache<::Kullo::Api::Messages>::getInstance()->get(cpp, [] (const CppType& p) {
        return [[KAMessages alloc] initWithCpp:p];
    });
}
